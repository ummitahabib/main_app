...
class _LikeAnimationWidgetState extends State<LikeAnimationWidget> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    _controller = AnimationController(
      vsync: this,
      duration: Duration(
        milliseconds: widget.duration.inMilliseconds ~/ SpacingConstants.int2,
      ),
    );
    scale = Tween<double>(
      begin: SpacingConstants.double1,
      end: SpacingConstants.double1Point2,
    ).animate(_controller);
    super.initState();
  }
  ...
  beginLikeAnimation() async {
    if (widget.isLikeAnimating) {
      ...
      await Future.delayed(
        const Duration(milliseconds: SpacingConstants.int200),
      );
